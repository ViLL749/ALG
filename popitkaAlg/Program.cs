...
namespace Project
{
    class Program
    {
        static List<string[]> userAnswersList = new List<string[]>(); // List to store user answers
        ...
        static void RunTest()
        {
            ...
            userAnswersList.Clear(); // Очищаем список с ответами пользователя после запуска теста
        ...
        static int TakeTest(string[] questions, string[][] correctAnswers)
        {
            ...
            for (int i = 0; i < questions.Length; i++)
            {
                ...
                userAnswersList.Add(userAnswers); // Store user answers in the list
        ...
        static void ShowTeacherResults(int correctAnswersCount, int totalQuestions, string[] questions, string[][] correctAnswers)
        {
            ...
            if (input == password)
            {
                ...
                {
                    string question = questions[i];
                    string[] userAnswers = userAnswersList[i]; // Получаем ответы пользователя
